...
def home(request):
	'''
	[basic Plaid home page, also for redirection]
	@/plaid/home/
	[GET req : Plaid Public token is fetched from Plaid-Web front-end]
	'''
...
class getAccessToken(APIView):
	...
	'''
	[To retrieve the access token to a item, after Plaid-link]
...
def transactionWebhook(request):
	...
	[INITIAL_UPDATE, HISTORICAL_UPDATE, DEFAULT_UPDATE => all are handled by get_transactions
	TRANSACTIONS_REMOVED => is handled by delete_transactions]
	@via Async
	Message Queue : Rabbitmq must have a worker running
	'''
